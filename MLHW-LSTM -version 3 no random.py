...
# change don't random
def getTrainBatch(train_data, ids, batchSize, maxSeqLength, batchid):
    labels = []
    batch_seqlen = []
    arr = np.zeros([batchSize, maxSeqLength])
    if batchid==len(train_data):
        batchid = 0
    rd = []
    for i in range(batchSize):
        if batchid + i >= len(train_data):
            rd.append(batchid + i - len(train_data))
        else:
            rd.append( batchid + i)
        num = rd[i]
        if train_data[num][0] == 'realDonaldTrump':
    ...
    new_batchid = rd[batchSize-1]+1
    #print(new_batchid)
    return arr, labels, batch_seqlen, new_batchid
#
# def getTrainBatch(train_data, ids, batchSize, maxSeqLength):
#     labels = []
#     batch_seqlen = []
...
for i in range(iterations):
   ...
   nextBatch, nextBatchLabels, nextseqlen,batchid = getTrainBatch(train_data,ids,batchSize,maxSeqLength,batchid);
